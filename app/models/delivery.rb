class Delivery < ApplicationRecord
  ...
  validates :delivery_type, inclusion: { in: %w[standard express pickup], message: "%{value} is not a valid delivery type" },
                            allow_blank: true
  validates :payment_type, inclusion: { in: %w[cash card online], message: "%{value} is not a valid payment type" },
                           allow_blank: true
  ...
  validates :delivery_notes, length: { maximum: 500 }, allow_blank: true
